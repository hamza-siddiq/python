...
def about(topic):
    ...
    def contains_words(paragraph):
        paragraph_words = split(remove_punctuation(paragraph))
        for i in topic:
            for j in paragraph_words:
                if i == lower(j):
                    return True
        return False
...
def wpm(typed, elapsed):
    ...
    return (len(typed) / 5) / elapsed_minutes
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    Arguments:
        ...
        diff_function: a function quantifying the difference between two words, takes in three arguments(typed_word,word from valid_words, limit)
        limit: a number, if lowest_diff is > limit --> return typed_word
    ...
    for valid_word in valid_words:
        if diff_function(typed_word, valid_word, limit) <= limit:
            if valid_word == typed_word:
                return typed_word
            if saved_word == "" or diff_function(typed_word, valid_word, limit) < diff_function(typed_word, saved_word, limit):
                saved_word = valid_word
    if len(saved_word) > 0:
        return saved_word
    return typed_word
    # END PROBLEM 5
