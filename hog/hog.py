...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    decimal_expansion = total = 142857
    index = score % 6
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        total = picky_piggy(opponent_score)
    else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 1:
            ...
            score = score0
        current_score = score - last_score
        if current_score > high_score:
            high_score = current_score
            print("{0} point(s)! That's a record gain for Player {1}!".format(high_score, who))
        return announce_highest(who, score, high_score)
    return say
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hog_check_score = score + picky_piggy(opponent_score)
    if hog_pile(hog_check_score, opponent_score):
        return 0
